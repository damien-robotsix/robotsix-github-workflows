...
- Remove `pre-commit` ecosystem from Dependabot config — the repo has no `.pre-commit-config.yaml`,
  causing the weekly update job to fail with "dependency_file_not_found".
