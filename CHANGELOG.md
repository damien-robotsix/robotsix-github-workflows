...
- `python-ci.yml`: run `coverage combine` before uploading the `coverage-data` artifact so the `.coverage` database is always included — fixes `No data to report.` failures in `python-coverage-comment-action` for consumers with `[tool.coverage.run] parallel = true`.
- Fix pre-existing zizmor `--persona=pedantic` error-level findings across six workflow files: replace spoofable `github.actor` bot checks with `github.event.pull_request.user.login` in `dependabot-auto-merge.yml`, add explicit `repositories:` scoping to every `actions/create-github-app-token` call, and move `docker-release.yml` permissions from workflow-level to job-level with `permissions: {}` as the default-deny baseline.
